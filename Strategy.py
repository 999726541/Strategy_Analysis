...
class Strategy_Base(object):
    ...
    def _equity(self,longest_period):
    ...
    def _drawdown(self,longest_period):#checked
        #回撤%
        length = len(self._df)
        for i in range(longest_period,len(self._df)):
    ...
    def _win_lose(self,longest_period):
        inital,final=0,0
        for i in range(longest_period, len(self._df)):
    ...
    def _return_rate(self,longest_period):
        for i in range(longest_period,len(self._df)):
            self._df.loc[i:i + 1, 'return'] = self._df.equity[i]/self.initial_invest


    def long_or_short(self,longest_period=120):
    ...
    def _beta(self,longest_period):
        for i in range(longest_period,len(self._df)):
            self._df.loc[i:i+1,'beta']=self._df.close[i]/self._df.close[longest_period]

    def backtest(self, longest_period=120):
        ###Order matters
        ...
        self.long_or_short(longest_period = longest_period)
        self._equity(longest_period = longest_period)
        self._win_lose(longest_period = longest_period)
        #f.write('买入次数: '+str(self.long_count)+ '\n')
        ##f.write('卖出次数: '+str(self.short_count)+ '\n')
        self._drawdown(longest_period = longest_period)
        self._return_rate(longest_period = longest_period)
        self.long_short_count()
        self._beta(longest_period = longest_period)
    ...
    def today_signal(self,longest_period):
        self.if_bear(longest_period=longest_period)
        self.if_bull(longest_period=longest_period)
        self.if_monkey(longest_period=longest_period)
        self.long_or_short(longest_period=longest_period)
        print(self._df[len(self._df)-1:len(self._df)].loc[:,['monkey','bull','bear','long','short']])
