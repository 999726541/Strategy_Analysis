...
class hs_300_strategy(Strategy_Base):
    ...
    def _if_long(self,i,ma_1,ma_2):
        ...
        if_pct_long:1 or 0
        '''
        if  self._df['ma_'+ma_1+'_close'][i] > self._df['ma_'+ma_2+'_close'][i] and \
                        self._df['ma_'+ma_1+'_close'][i-1] < self._df['ma_'+ma_2+'_close'][i-1]:
            #self._df.loc[i:,'P_new'] = (self._df['ma_'+ma_1+'_close'][i]+self._df['ma_'+ma_2+'_close'][i]+
            #                            self._df['ma_' + ma_1 + '_close'][i - 1] + self._df['ma_' + ma_2 + '_close'][
            #                                i - 1])/4
            return True,1,1,1
    ...
    def _if_short(self,i,ma_1,ma_2):
        ...
        if self._df['ma_' + ma_1 + '_close'][i] < self._df['ma_' + ma_2 + '_close'][i] and \
            ...
            return True, 1, 1
        #elif (self._df['ma_' + ma_1 + '_close'][i] + self._df['ma_' + ma_2 + '_close'][i])/2 < self._df['P_new'][i]:
            #self._df.loc[i:, 'P_new'] = 0
            #return True, 1, 1
        else:
            return False,1,1
...
#
    ...
    df = get_his_data('hs300', ma=ma, start='2015-07-01', end='2015-09-31')
    test_4 = hs_300_strategy(df)
    all_return_4 = {}
    resu = test_(ma, all_return_4, test_4)
    resu.to_csv('ma_test_result.csv', mode='a')
#
    df = get_his_data('hs300', ma=ma, start='2015-10-01', end='2015-12-31')
    test_4 = hs_300_strategy(df)
    all_return_4 = {}
    resu = test_(ma, all_return_4, test_4)
    resu.to_csv('ma_test_result.csv', mode='a')

    df = get_his_data('hs300', ma=ma, start='2016-01-01', end='2016-03-31')
    test_4 = hs_300_strategy(df)
    all_return_4 = {}
    resu = test_(ma, all_return_4, test_4)
    resu.to_csv('ma_test_result.csv', mode='a')

    df = get_his_data('hs300', ma=ma, start='2016-04-01', end='2016-06-31')
    test_4 = hs_300_strategy(df)
    all_return_4 = {}
    resu = test_(ma, all_return_4, test_4)
    resu.to_csv('ma_test_result.csv', mode='a')

    df = get_his_data('hs300', ma=ma, start='2016-07-01')
    test_4 = hs_300_strategy(df)
    all_return_4 = {}
    resu = test_(ma, all_return_4, test_4)
    resu.to_csv('ma_test_result.csv', mode='a')"""